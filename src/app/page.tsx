...
export default function LandingPage() {
  ...
  return (
    <div className="bg-brandBg text-white antialiased font-sans overflow-x-hidden selection:bg-brandGreen selection:text-black min-h-screen relative">
      ...
      <main className="relative z-10 pt-20">
        <section className="min-h-screen flex flex-col items-center justify-center px-6 relative overflow-hidden py-20">
          ...
          <div id="signup" className="w-full max-w-md mx-auto mt-12 z-20 reveal-element active space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <AuthButton provider="Apple" className="inline-flex items-center justify-center gap-2 py-3.5 px-3 bg-brandSurface hover:bg-brandSurfaceAccent border border-brandBorder rounded-pill text-sm font-medium transition-all duration-300 hover:scale-[1.02] focus:outline-none focus:ring-1 focus:ring-white/20"><svg className="w-4 h-4 text-white shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M15.97 4.17c.66-.81 1.11-1.93.99-3.06-.96.04-2.13.64-2.82 1.45-.6.69-1.12 1.84-.98 2.94 1.07.08 2.15-.52 2.81-1.33z" /></svg><span>Apple</span></AuthButton>
              <AuthButton provider="Facebook" className="inline-flex items-center justify-center gap-2 py-3.5 px-3 bg-brandSurface hover:bg-brandSurfaceAccent border border-brandBorder rounded-pill text-sm font-medium transition-all duration-300 hover:scale-[1.02] focus:outline-none focus:ring-1 focus:ring-white/20"><svg className="w-4 h-4 text-[#1877F2] shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.84 3.44 8.87 8 9.8V15H8v-3h2V9.5C10 7.57 11.57 6 13.5 6H16v3h-2c-.55 0-1 .45-1 1v2h3v3h-3v6.8c4.56-.93 8-4.96 8-9.8z" /></svg><span>Facebook</span></AuthButton>
            </div>

            <div className="flex items-center justify-center gap-4 text-xs font-bold tracking-widest text-brandTextSec"><div className="h-[1px] bg-brandBorder flex-1" /><span>OR</span><div className="h-[1px] bg-brandBorder flex-1" /></div>
        ...
        <section id="features" className="py-32 px-6 max-w-7xl mx-auto relative z-20">
          <Reveal className="text-center max-w-2xl mx-auto mb-20">
            ...
            <p className="mt-4 text-brandTextSec leading-relaxed">QuickStart.Ai models code across your entire ecosystem simultaneously. No disconnected outputs. Pure production excellence.</p>
          </Reveal>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {FEATURES.map((f) => (
              <Reveal key={f.title} className="glass-card rounded-premium p-8 flex flex-col justify-between min-h-[300px]">
                <div>
                  <div className="w-12 h-12 rounded-full bg-brandSurfaceAccent flex items-center justify-center text-brandGreen border border-brandBorder"><f.icon className="w-5 h-5" /></div>
                ...
                <div className="mt-8 flex items-center gap-2 text-xs font-semibold tracking-wider text-brandGreen uppercase"><span>{f.tag}</span></div>
              </Reveal>
            ))}
          </div>

          <Reveal className="mt-20 border-t border-b border-brandBorder py-12">
            <p className="text-center text-xs uppercase tracking-[0.2em] font-bold text-brandTextSec mb-10">Export native platforms & framework models</p>
            <div className="flex flex-wrap justify-center gap-4 max-w-4xl mx-auto">
              {TECH_TAGS.map((tag) => <span key={tag} className="px-5 py-2.5 rounded-pill bg-brandSurface border border-brandBorder text-sm font-medium hover:border-brandGreen transition-colors duration-200">{tag}</span>)}
            </div>
          </Reveal>
        </section>
      </main>
      ...
      <style>{`
        .noise-bg { position: fixed; top: -50%; left: -50%; right: -50%; bottom: -50%; width: 200%; height: 200%; background: transparent url('data:image/svg+xml,%3Csvg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg"%3E%3Cfilter id="noiseFilter"%3E%3CfeTurbulence type="fractalNoise" baseFrequency="0.65" numOctaves="3" stitchTiles="stitch"/%3E%3C/filter%3E%3Crect width="100%25" height="100%25" filter="url(%23noiseFilter)" opacity="0.015"/%3E%3C/svg%3E') repeat; opacity: 0.8; pointer-events: none; z-index: 999; animation: noise-anim 0.2s infinite; }
